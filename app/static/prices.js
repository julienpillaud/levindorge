window.addEventListener("DOMContentLoaded", function () {
    ...
    sellPriceArray.forEach(function (elem) {
        ...
        elem.disabled = window.getComputedStyle(elem.closest(".row"), null).display === "none";
...
function calculateTaxfreePrice() {
    ...
    let social_security_levy_value = social_security_levy ? social_security_levy.value : 0;

    let value = Number(buy_price_value) + Number(excise_duty_value) + Number(social_security_levy_value);
...
function fetchRecommendedPrices() {
    ...
    return fetch("/articles/recommended_prices", options)
...
function fillRecommendedPrices(shop, recommendedPrice, on_load) {
    ...
    document.getElementById("_recommended_price_" + shop).value = recommendedPrice;
    if (!on_load) {
...
function fetchMargins(sellPrice) {
    ...
    return fetch("/articles/margins", options)
        .then(response => response.json());
}
...
function colorPrices(shop, markup) {
    let recommendedPrice = document.getElementById("recommended_price_" + shop).value;
